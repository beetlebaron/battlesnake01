...
def calculate_move(board_matrix, game_state):
    ...
    # Check down
    if head["y"] + 1 > (height) - 2 or (board_matrix[y+1][x] ) == OCCUPIED : #and board_matrix[y+2][x] and board_matrix[y+2][x+1] and board_matrix[y+2][x-1]
    ...
    # Check Left
    if head["x"] - 1 < 0+1 or (board_matrix[y-1][x-2] ) == OCCUPIED : #and board_matrix[y][x-2] and board_matrix[y+1][x-2] and board_matrix[y][x-1]
